            ...
            <Divider style={{ margin: '16px 0' }} />
              ...
              <div className={classes.flexForMessage}>
                ...
                <form className={classes.flexForForm} autoComplete='off' noValidate onSubmit={handleSubmit}>
                    <TextField className={classes.marginLeft} fullWidth multiline value={message} onChange={handleChange} onBlur={toggleSubmitVis} onFocus={()=>{if(!signedIn){navigate('/auth')}else{toggleSubmitVis()}}} label="Leave a comment..." variant="filled" size='small' color='secondary' />
              ...
              {comments.map(comment => {
                  ...
                  <Avatar className={classes.avatar} onClick={() => navigate(`/users/${comment.creator}`)}>{comment.name.charAt(0).toUpperCase()}</Avatar>
                  <div className={classes.userStats}>
                      <div className={classes.flexForNameDate}>
                        <Typography variant='body2' className={classes.name} onClick={() => navigate(`/users/${comment.creator}`)} >{comment.name}</Typography>
                        <Typography variant='body2' className={classes.date} >{dayjs(comment.createdAt).fromNow()}</Typography>
                      </div>
