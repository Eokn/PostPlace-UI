...
const Home = () => {
  ...
  const searchPosts = () => {
    if(search.trim() || tags.length !== 0 ) {
      navigate(`/posts/search?searchQuery=${search || 'none'}&tags=${tags.join(',')}&page=1`)
    ...
    return (
        <Grow in>
            <Container className={classes.appContainer} maxWidth='xl'>
              <Grid container sx={{ justifyContent: 'space-between', alignItems: 'stretch' }} spacing={2} className={`${!signedIn ? classes.notSignedIn : ''} ${classes.gridContainer}`} >
                {!signedIn && (<Grid><AppBar position='static' color='inherit' className={classes.appBarSearch} >
                    <TextField name='search' className={classes.titleCorrect} slotProps={{inputLabel: { className: classes.formPos }}}  color='secondary' margin='dense' variant='outlined' label='Search Posts' value={search} onKeyPress={handleKeyPress} onChange={(e)=>{setSearch(e.target.value)}} />
