    ...
    return (
        <Paper className={classes.paperContainer} elevation={6}>
            ...
            <div key={x._id} className={classes.activity}  >
                ...
                <Typography gutterBottom variant='h6'>{`${ownPage ? 'You' : x.name} created '${x.title}'`}<span className={classes.dateText} >{dayjs(x.createdAt).fromNow()}</span></Typography>
                <Post post={x} userPage onClick={() => navigate(`/posts/${x._id}`)} cursor='pointer' />
            </div>
            ) : (
            <div key={x._id} className={classes.activity} >
                ...
                <Typography gutterBottom variant='h6'>{`${ownPage ? 'You' : x.name} left a comment`}<span className={classes.dateText} >{dayjs(x.createdAt).fromNow()}</span></Typography>
                <Typography variant='body1' className={classes.textPop} onClick={() => navigate(`/posts/${x.belongsTo}`)} >{x.message}</Typography>
            </div>
            ) )}
